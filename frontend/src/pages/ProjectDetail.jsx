...
const TABS = [
  ...
  { id: "carbon", label: "Carbon Analytics", icon: BarChart3 },
  { id: "health", label: "Vegetation Health", icon: Activity },
...
export default function ProjectDetail() {
  ...
  useEffect(() => {
    api
      ...
      .then((data) => {
        setSites(data);
        const match = data.find((s) => s.id === projectId);
        if (match?.blocks?.length) setBlockName(match.blocks[0]);
      })
      .catch(() => {});
  }, [projectId]);
  ...
  useEffect(() => {
    if (siteId === "custom" && !customCoords) return;
    if (siteId === "sundari" && !blockName) return; // wait for block to resolve
    runAnalysis(false);
    ...
    <div className="min-h-screen px-4 py-6 sm:px-8">
      <header className="mx-auto mb-6 flex max-w-7xl items-center justify-between">
        <div>
          <button
            onClick={() => navigate("/")}
            className="mb-2 flex items-center gap-1.5 text-xs text-muted hover:text-ink"
          >
            <ArrowLeft size={14} /> All projects
          </button>
          <h1 className="title-gradient font-display text-3xl font-bold sm:text-4xl">
            {result?.project_name || "Project Overview"}
          </h1>
          <p className="text-sm text-muted">
            Continuous biophysical tracking and carbon density estimation from remote sensing
          </p>
        </div>
      </header>
        ...
        <main className="flex-1">
          ...
          {loading ? (
            ...
            <div className="glass-card px-6 py-8">
              ...
              <InfoBox>
                Try selecting a different project, or draw a polygon over a known coastal mangrove/wetland
                zone.
              </InfoBox>
            </div>
          ) : result ? (
            <>
              ...
              <div className="glass-card p-5 sm:p-6">
                ...
                {activeTab === "map" && (
                  <>
                    <InfoBox>
                      Draw a custom boundary directly on the map below to monitor a new area within this
                      project.
                    </InfoBox>
                    <MonitoringMap
