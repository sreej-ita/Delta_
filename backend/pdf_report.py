...
class EcosystemReport(FPDF):
    def header(self):
        ...
        # Draw the rule based on where the text actually ended, not a fixed
        # y-coordinate — this is what was causing the line to cut through
        # the subtitle text when the header text ran slightly taller than
        # the hardcoded y=20 assumed.
        self.ln(1)
        line_y = self.get_y()
...
def generate_pdf_report(project_name, analysis_data, carbon_data, project_meta=None, checklist=None):
    ...
    # --- Disclaimer box ---
    # The box height is measured from the ACTUAL wrapped text (via
    # split_only=True) instead of a hardcoded value, so the text can never
    # overflow the box regardless of font metrics or content length. The box
    # and text also now share the same x-position/width so the text is
    # properly centered and padded inside the box, rather than the text
    # being drawn from the page's left margin while the box sat elsewhere.
    disclaimer_text = (
    ...
    box_x, box_w = 20, 170
    pad_x, pad_y = 5, 4
    text_w = box_w - (2 * pad_x)
    line_h = 4
 
    pdf.set_font('Helvetica', 'B', 8.5)
    wrapped_lines = pdf.multi_cell(text_w, line_h, _pdf_safe(disclaimer_text), align='C', split_only=True)
    box_h = (2 * pad_y) + (len(wrapped_lines) * line_h)
    ...
    pdf.multi_cell(text_w, line_h, _pdf_safe(disclaimer_text), align='C')
